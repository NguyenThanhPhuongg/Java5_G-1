    ...
    <form action="/san-pham-chi-tiet/update/${data.id}" method="post">
        ...
        <div class="mb-3">
            ...
            <select class="form-select" name="idSP">
                ...
                <option value="${sanPham.id}" <c:if test="${data.idSP == sanPham.id ? 'selected':''}"></c:if> >${sanPham.ten}</option>
                </c:forEach>
            </select>
        ...
        <div class="mb-3">
            ...
            <select class="form-select" name="idKT">
                <c:forEach items="${listKichThuoc}" var="kichThuoc">
                    <option value="${kichThuoc.id}" <c:if test="${data.idKT == kichThuoc.id ? 'selected':''}"></c:if>>${kichThuoc.ten}</option>
                </c:forEach>
            </select>
            <%--            <c:if test="${not empty errors['ten']}">--%>
            <%--                <small style="color: red">${errors['ten']}</small>--%>
        ...
        <div class="mb-3">
            ...
            <select class="form-select" name="idMS">
                <c:forEach items="${listMauSac}" var="mauSac">
                    <option value="${mauSac.id}" <c:if test="${data.idMS == mauSac.id ? 'selected':''}"></c:if>>${mauSac.ten}</option>
                </c:forEach>
            </select>
            <%--            <c:if test="${not empty errors['ten']}">--%>
            <%--                <small style="color: red">${errors['ten']}</small>--%>
